...
public final class LockStoreImpl implements DataSerializable, LockStore {
    ...
    public boolean lock(Data key, UUID caller, long threadId) {
        return lock(key, caller, threadId, Long.MAX_VALUE);
    }

    @Override
    public boolean lock(Data key, UUID caller, long threadId, long leaseTime) {
        LockResourceImpl lock = getLock(key);
        return lock.lock(caller, threadId, leaseTime);
    }

    @Override
    public boolean txnLock(Data key, UUID caller, long threadId, long leaseTime) {
        LockResourceImpl lock = getLock(key);
